    ...
    F: Fn() + 'static + Clone + Send + Sync,
{
    ...
    // Pre-clone for the submit handler.
    let on_switch_submit = on_switch.clone();

    let handle_submit = move |ev: web_sys::SubmitEvent| {
        let _ = &on_switch_submit;
        ev.prevent_default();
    ...
    // Give the reactive block its own clone. F: Clone so .clone() is cheap.
    // The reactive closure stays FnMut because it only calls .clone() each
    // evaluation — it never moves on_switch_view itself.
    let on_switch_view = on_switch.clone();

    view! {
        <div class="auth-form-wrap">

            {move || {
                let os_confirmed = on_switch_view.clone();
                let os_switch    = on_switch_view.clone();
                ...
                if confirmed.get() {
                    ...
                    view! {
                        ...
                        <div class="auth-switch">
                            ...
                            <button class="link-btn" on:click=move |_| os_switch()>
                                "Sign in"
                            </button>
                        </div>
                    }.into_any()
